  ...
  if Trophy.find_by(variant:).nil? then
    ...
    if [unnethack, evilhack].include? variant
      ...
      if [evilhack].include? variant then
        ...
        achievements << [:better_than_croesus,  "Better than Croesus (finish a game with at least 200,000 gold pieces)", "better_than_croesus.png", 6]
      end
    ...
    if [nethack37, unnethack, splicehack, xnethack, gnollhack, nerfhack, crecellehack].include? variant then
      ...
      if ![gnollhack].include?(variant)
        achievements << [:mini_croesus, "Mini-Croesus (finish a game with at least 25,000 gold pieces)", 3]
        achievements << [:croesus_buddy, "Croesus' buddy (finish a game with at least 100,000 gold pieces)", 3]
        achievements << [:better_than_croesus, "Better than Croesus (finish a game with at least 200,000 gold pieces)",
                         3]
      end
    ...
    if [dnethack, notdnethack, notnotdnethack].include? variant then
      achievements = [
        ...
        [:knight_quest, "Into the crystal cave (Completed the revised knight quest)", "dnh_knight_quest.png", 6],
        [:anachrononaut_quest, "Back from the future (Completed the anachrononaut quest)", "dnh_anachrononaut_quest.png", 6],
        ...
        [:dwarf_noble_quest, "Durin's Bane's Bane (Completed the dwarf noble quest)", "dnh_dwanob.png", 6],
        [:dwarf_knight_quest, "Battle of (5-4) armies (Completed the dwarf knight quest)", "dnh_dwarf_knight_quest.png", 6],
        [:gnome_ranger_quest, "Strongest of all time (Completed the gnome ranger quest)", "dnh_gnome_ranger_quest.png", 6],
        ...
        [:chaos_temple_quest, "Asinine paradigm (Completed the chaos temple quest)", "dnh_chaos1_quest.png", 6],
        [:mithardir_quest, "Chasing after the wind (Completed the mithardir quest)", "dnh_mithardir.png", 6],
        [:mordor_quest, "Fossil of the First Age (Completed the mordor quest)", "dnh_mordish.png", 6],
        [:second_thoughts, "Second thoughts (Completed a drow shared quest loyally before completing the traitor's quest)", "dnh_second.png", 5],
        ...
        [:lamashtu_kill, "Does this count as a paradox? (Destroyed Lamashtu (Demon Queen of the future))", "dnh_lamashtu.png", 5],
        [:baalphegor_kill, "A universe without motion (Destroyed Baalphegor)", "dnh_baalphegor.png", 5],
        [:bokrug_ascension, "Detestable gods (Completed Bokrug's ascension ritual)", "dnh_bokrug_asc.png", 5],
        [:healer_quest, "Plague of stolen lives (Completed healer quest)", "dnh_healer_qst.png", 6],
    ...
    if [xnethack].include? variant then
      Trophy.create variant: variant, trophy: :killed_all_archfiends, text: "Thoroughly explore Gehennom and kill all eight archfiends", icon: "the_purge_of_pandemonium.png", row: 4
      Trophy.create variant: variant, trophy: :completed_arc_quest, text: "completed the revised Archeologist quest", icon: "completed_arc_quest.png", row: 4
      Trophy.create variant: variant, trophy: :completed_mon_quest, text: "completed the revised Monk quest", icon: "completed_mon_quest.png", row: 4
    ...
    if [slashem].include? variant then
      ...
      achievements << [:croesus_buddy,         "Croesus' buddy (finish a game with at least 100,000 gold pieces)", "croesus_buddy.png", 2]
      achievements << [:better_than_croesus,   "Better than Croesus (finish a game with at least 200,000 gold pieces)", "better_than_croesus.png", 2]
