...
describe Game,"update_clan_scores" do
  ...
  it "includes NetHack 1.3d games" do
    ...
    ClanScoreEntry.find_by(trophy: "most_ascended_combinations").value.should == 1
    ClanScoreEntry.find_by(trophy: "most_ascensions_in_a_24_hour_period").value.should == 1
    ClanScoreEntry.find_by(trophy: "most_full_conducts_broken").value.should == 0
    ClanScoreEntry.find_by(trophy: "most_medusa_kills").value.should == 0
    ClanScoreEntry.find_by(trophy: "most_unique_deaths").value.should == 1
    ClanScoreEntry.find_by(trophy: "most_variant_trophy_combinations").value.should == 1
  end
  ...
  it "should correctly calculate clan scores" do
    ...
    ClanScoreEntry.find_by(trophy: "most_ascended_combinations").value.should == 2
    ClanScoreEntry.find_by(trophy: "most_ascensions_in_a_24_hour_period").value.should == 3
    ClanScoreEntry.find_by(trophy: "most_full_conducts_broken").value.should == 2
    ClanScoreEntry.find_by(trophy: "most_medusa_kills").value.should == 2
    ClanScoreEntry.find_by(trophy: "most_unique_deaths").value.should == 2
    ClanScoreEntry.find_by(trophy: "most_variant_trophy_combinations").value.should == 9
  end

  it "creates clan score history entries" do
    Game.create(server_id: @server.id, version: 'v1', achieve: "0x800", points: 9000, endtime: 1000, death: 'ascended', turns: 1023)
    update_games
    ClanScoreEntry.find_by(trophy: :most_medusa_kills).value.should == 1
    ClanScoreHistory.where(trophy: :most_medusa_kills).count.should == 1
  end
  ...
  it "creates clan score history entries if the value of the trophy changes" do
    ...
    ClanScoreEntry.find_by(trophy: :most_medusa_kills).value.should == 2
    ClanScoreHistory.where(trophy: :most_medusa_kills).count.should == 2
    ClanScoreHistory.where(trophy: :most_medusa_kills).map(&:value).sort == [1, 2]
  end
  ...
  describe 'lowest turns getting killed by monsters' do
    ...
    it 'creates the clan trophy' do
      pending
      expect(ClanScoreEntry.find_by(trophy: :lowest_turns_for_monster_kills)).not_to be

      Game.create(attributes.merge(turns: 256, death: 'killed by Vlad the Impaler'))
      update_games

      expect(ClanScoreEntry.find_by(trophy: :lowest_turns_for_monster_kills)).to be
      expect(ClanScoreEntry.find_by(trophy: :lowest_turns_for_monster_kills).value).to eq 511
    end
  end
end
